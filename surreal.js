...
var $ = {
    // You can use a different name than "$", but you must change the reference in any plugins you use!
    ...
    sugar(e) {
        if (e == null) {
            console.warn(`Cannot use "${e}". Missing a character?`);
        }
    ...
    me(selector = null, start = document, warning = true) {
        ...
        if (typeof selector == 'string' && isSelector(selector, start, warning))
    ...
    any(selector, start = document, warning = true) {
        if (selector == null)
            return $.sugar([start.currentScript.parentElement]); // Just local me() in <script>
        if (selector instanceof Event) return $.any(selector.target); // Events return event.target
        if (
            typeof selector == 'string' &&
            isSelector(selector, start, true, warning)
        )
    ...
    isSelector(selector = '', start = document, all = false, warning = true) {
        if (all && start.querySelectorAll(selector) == null) {
            if (warning)
                console.warn(
                    `"${selector}" was not found. Missing a character? (. #)`
                );
            return false;
        }
